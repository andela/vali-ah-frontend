...
const {
  ...
  createCommentSuccess, createCommentFailure, makeCreateCommentAPICall, makeGetCommentsAPICall
} = actions;
...
describe('Comment actions', () => {
  ...
  it('should dispatch action to call api on success', async () => {
    ...
    const dispatched = await recordSaga(makeGetCommentsAPICall, initialAction);

    expect(getStub.calledOnce).toBe(true);
    expect(dispatched[0].type).toEqual('vali-ah-frontend/auth/GET_COMMENTS_SUCCESS');
  });
  ...
  it('should dispatch action on failure', async () => {
    ...
    expect(getStub.calledOnce).toBe(true);
    expect(dispatched[0].type).toEqual('vali-ah-frontend/auth/GET_COMMENTS_FAILURE');
  });
  ...
  it('should dispatch create comment api on success', async () => {
    ...
    const dispatched = await recordSaga(makeCreateCommentAPICall, initialAction);

    expect(postStub.calledOnce).toBe(true);
    expect(dispatched[0].type).toEqual('vali-ah-frontend/auth/CREATE_COMMENT_SUCCESS');
  });
  ...
  it('should dispatch create comment api on success', async () => {
    ...
    expect(postStub.calledOnce).toBe(true);
    expect(dispatched[0].type).toEqual('vali-ah-frontend/auth/CREATE_COMMENT_FAILURE');
  });
});
